    ...
    <div class="card card-default px-1 py-1">
        ...
        <div class="col-md-12">
            <div class="form-group">
                <label>Unlock Reason <span class="span_red">*</span></label>
                <textarea class="form-control" id="unlock_reason" maxlength="250" name="unlock_reason" tabindex="1" ></textarea>
                <span id="unlock_reason-error" class="error">Please Input Unlock Reason</span>
            </div>
        </div>
        <div class="card-footer">
            <button tabindex="2" type="button" id="return" onclick="validation();" class="btn btn-danger float-right">Unlock</button>
        </div>
    </div>
</div> 
...
<script type="text/javascript">
    ...
    function validation(){
        ...
        if(form_validation('unlock_reason#text','Unlock Reason')==false ){

            return false;
        }

        confirm_box('Unlock', 'Are You Sure Want To Unlock?','Unlock');
    }
    ...
    function Unlock(){
        ...
        $.ajax({
            ...
            type: "POST",
            data: {
                unlock_reason: $('#unlock_reason').val()
            },
            ...
            success: function(response) {
                ...
                all_response(response,'{{route(request()->route()->getName())}}','Unlock View Employee/User');
            },
